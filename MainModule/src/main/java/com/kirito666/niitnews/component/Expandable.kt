...
import com.kirshi.freya.ui.component.ExpandableIconStyles
...
class Expandable : FrameLayout {
    ...
    private fun initTypedArray(attrs: AttributeSet?) {
        ...
        iconStyle = ExpandableIconStyles.getByIndex(
            ta.getInt(
                R.styleable.Expandable_exp_iconStyle,
                ExpandableIconStyles.SQUARE.ordinal
            )
        )
        animateExpand = ta.getBoolean(R.styleable.Expandable_exp_animateExpand, false)
        backgroundColor = ta.getColor(
            R.styleable.Expandable_exp_backgroundColor,
    ...
    private fun createHeaderIcon() {
        ...
        iconImage.setImageDrawable(
            when (iconStyle) {
                ExpandableIconStyles.CIRCLE -> icon?.getCircleBitmap(backgroundColor)
                    ?.toDrawable(resources)
                ExpandableIconStyles.ROUNDED_SQUARE -> icon?.getRoundedSquareBitmap(backgroundColor)
                    ?.toDrawable(resources)
                else -> icon
            }
        )
        ...
        iconImage.apply {
            layoutParams =
                LayoutParams(iconSize, iconSize, Gravity.START or Gravity.CENTER_VERTICAL)
    ...
    private fun createExpandIcon() {
        ...
        expandIcon.apply {
            ...
            setImageDrawable(
                if (expandIndicator == null) ContextCompat.getDrawable(
                    context,
                    R.drawable.ic_down
                ) else expandIndicator
            )
        }
    ...
    private fun createHeaderContent() {
        ...
        val indicatorIconSize =
            context.resources.getDimensionPixelSize(R.dimen.expand_drawable_size)
    ...
    fun collapse() {
        if (animateExpand) {
            ...
            iconImage.animate().scaleX(1f).scaleY(1f).translationX(1f).translationY(1f).duration =
                200
    ...
    fun expand() {
        ...
        if (animateExpand) {
            ...
            iconImage.animate().scaleX(1.2f).scaleY(1.2f).translationX(10f)
                .translationY(10f).duration = 200
