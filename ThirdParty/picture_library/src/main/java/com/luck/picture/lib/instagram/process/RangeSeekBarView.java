 ...
 * @Project:NiitNews
 * @Author:Finger
 * @FileName:RangeSeekBarView.java
...
public class RangeSeekBarView extends View {
    ...
    private long mMinShootTime = 3000L;
...
//    private Bitmap thumbPressedImage;
    ...
    public RangeSeekBarView(Context context, long absoluteMinValuePrim, long absoluteMaxValuePrim, int thumbsCount) {
        super(context);
    ...
    private void init() {
        TextPositionY = ScreenUtils.dip2px(getContext(), 7);
        paddingTop = ScreenUtils.dip2px(getContext(), 10);
...
//        int height = thumbImageLeft.getHeight();
        int newWidth = ScreenUtils.dip2px(getContext(), 20);
...
//        thumbPressedImage = thumbImageLeft;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mThumbsCount < 8) {
            int frameWidth = (ScreenUtils.getScreenWidth(getContext()) - ScreenUtils.dip2px(getContext(), 40)) / 8;
            float maxWidth = frameWidth * mThumbsCount + thumbWidth;
            if (width > maxWidth) {
                width = (int) maxWidth;
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Rect leftRect = new Rect((int) (bg_middle_left + thumbHalfWidth), getHeight(), (int) (rangeL + thumbHalfWidth), 0);
        Rect rightRect = new Rect((int) (rangeR - thumbHalfWidth), getHeight(), (int) (bg_middle_right - thumbHalfWidth), 0);
        canvas.drawRect(leftRect, mShadow);
        canvas.drawRect(rightRect, mShadow);

        canvas.drawRect(0, 0, thumbHalfWidth, getHeight(), mShadow);
        if (mThumbsCount >= 8) {
            canvas.drawRect(getWidth() - thumbHalfWidth, 0, getWidth(), getHeight(), mShadow);
        }

        canvas.drawRect(rangeL + thumbHalfWidth, 0, rangeR - thumbHalfWidth, thumbPaddingTop + ScreenUtils.dip2px(getContext(), 2), rectPaint);
...
//    drawVideoTrimTimeText(canvas);
    ...
    private void drawThumb(float screenCoord, boolean pressed, Canvas canvas, boolean isLeft) {
        float left = isLeft ? thumbHalfWidth + screenCoord : screenCoord - thumbHalfWidth;
        canvas.drawRect(left, 0, left + ScreenUtils.dip2px(getContext(), 2), getHeight(), rectPaint);
        canvas.drawCircle(left, getHeight() / 2, thumbHalfWidth, rectPaint);
